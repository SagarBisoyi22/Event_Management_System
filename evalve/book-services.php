...
$sql="insert into tblbooking(BookingID,ServiceID,UserID,BookingFrom,BookingTo,EventType,Numberofguest,Message,stateId,cityName)values(:bookingid,:bid,:uid,:bookingfrom,:bookingto,:eventtype,:nop,:message,:stateid,:cityname)";
...
<html lang="en">
<head>
<title>Evalve | Book Services</title>
...
<option value="<?php echo htmlentities($row1->id);?>"><?php echo htmlentities($row1->state_title);?></option>
 <?php } ?>
            ...
            <div class="form-group row">
                                    <label class="col-form-label col-md-4">State</label>
                                    <div class="col-md-10">
                                       <select type="text" class="form-control"   name="city-list" id="city-list" required="true" >
				 </select>
                                    </div>
                                </div>
